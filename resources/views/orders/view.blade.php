                        ...
                        <div class="card-body p-4">
                            <div class="row g-3">
                                ...
                                <div class="col-4">
                                    ...
                                    <h5 class="fs-14 mb-0"> <span class="text-muted">M/S :</span>
                                        {{ $order->customer->title }}</h5>
                                    @if ($order->customerID != 2)
                                        <h5 class="fs-14 mb-0"> <span class="text-muted">Area :</span>
                                            {{ $order->customer->area->name ?? 'NA' }} | <span class="text-muted">Contact
                                                :</span> {{ $order->customer->contact ?? 'NA' }}</h5>
                                        <h5 class="fs-14 mb-0"> <span class="text-muted">Type :</span>
                                            {{ $order->customer->c_type }}</h5>
                                        <h5 class="fs-14 mb-0"> <span class="text-muted">Address :</span>
                                            {{ $order->customer->address ?? 'NA' }}</h5>
                                    @endif
                                ...
                                <table class="table table-borderless text-center table-nowrap align-middle mb-0">
                                    ...
                                    <tbody id="products-list">
                                        ...
                                        @foreach ($order->details as $key => $product)
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-start p-1 m-1">{{ $product->product->name }} <br>
                                                    {{ $product->product->nameurdu }}</td>
                                                ...
                                                <td class="text-end p-1 m-1">{{ number_format($price) }}</td>
                                                <td class="text-end p-1 m-1">{{ number_format($discount) }}</td>
                                                <td class="text-end p-1 m-1">{{ $product->discountp }}% |
                                                    {{ number_format($discountvalue) }}</td>
                                                <td class="text-end p-1 m-1">{{ number_format($product->claim * $qty) }}
                                                </td>
                                                <td class="text-end p-1 m-1">{{ number_format($netprice) }}</td>
                                                <td class="text-end p-1 m-1">{{ number_format($product->fright * $qty) }}
                                                </td>
                                    ...
                                    <tfoot>
                                        <tr>
                                            ...
                                            <th class="text-end">{{ number_format($netAmount) }}</th>
                                        </tr>
                                    </tfoot>
                                </table><!--end table-->
                            </div>
                                ...
                                <div class="col-12">
                                    <table class="table">
                                        ...
                                        <tbody>
                                            @foreach ($order->details as $key => $product)
                                                <tr>
                                                    ...
                                                    <td class="m-1 p-1">
                                                        {{ packInfo($product->unit->value, $product->unit->unit_name, $product->pc) }}
                                                    </td>
                                                    <td class="m-1 p-1">
                                                        {{ packInfo($product->unit->value, $product->unit->unit_name, $product->delivered()) }}
                                                    </td>
                                                    <td class="m-1 p-1">
                                                        {{ packInfo($product->unit->value, $product->unit->unit_name, $product->remaining()) }}
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                        <tfoot>
                                            <tr>
                                                <th colspan="4" class="text-end">Total</th>
                                                <th>{{ $order->details->sum('qty') }}, {{ $order->details->sum('loose') }}
                                                </th>
                                                <th>{{ $order->delivered_items->sum('qty') }},
                                                    {{ $order->delivered_items->sum('loose') }}</th>
                                                <th>{{ $order->details->sum('qty') - $order->delivered_items->sum('qty') }},
                                                    {{ $order->details->sum('loose') - $order->delivered_items->sum('loose') }}
                                                </th>
                                            </tr>
                                        </tfoot>
                                    </table>
