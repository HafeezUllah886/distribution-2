...
class BranchOrdersController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        foreach ($methods as $method) {
            $sales_payment = sale_payments::where('customerID', $customer)->where('date', $newest_date)->where('method', $method)->where('orderbookerID', $orderbooker)->sum('amount');
            $payment_receiving = paymentsReceiving::where('depositerID', $customer)->where('date', $newest_date)->where('method', $method)->where('orderbookerID', $orderbooker)->sum('amount');

            $total = $sales_payment + $payment_receiving;
            $methodData[$method] = round($total, 2);
        }

        $last_sale = sales::where('customerID', $customer)->where('orderbookerID', $orderbooker)->orderBy('id', 'desc')->first();
        if ($last_sale) {
